...
int main() {
  ...
  while (true) {
    ...
    loop_cnt++;
    // printf("raw x: %.2f, y: %.2f, z: %.2f, r: %.2f, p: %.2f, y: %.2f, gripper: %.2f\n", eef_state.pose_6d[0], eef_state.pose_6d[1], eef_state.pose_6d[2], eef_state.pose_6d[3], eef_state.pose_6d[4], eef_state.pose_6d[5], eef_state.gripper_pos);
    ...
    std::tuple<bool, Vec6d> result =
        solver.inverse_kinematics(eef_state.pose_6d, joint_state.pos);
    if (std::get<0>(result)) {
