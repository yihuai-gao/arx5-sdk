...
int main()
{
    ...
    while (true)
    {
        ...
        std::tuple<bool, Vec6d> result = solver.inverse_kinematics(high_state.pose_6d, low_state.pos);
        if (std::get<0>(result))
        {
            Vec6d ik_joint_pos = std::get<1>(result);
            printf("ik  joint pos: %.2f, %.2f, %.2f, %.2f, %.2f, %.2f\n", ik_joint_pos[0], ik_joint_pos[1], ik_joint_pos[2], ik_joint_pos[3], ik_joint_pos[4], ik_joint_pos[5]);
        }
